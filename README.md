...
Для того, чтобы "BuildAndTest" давал нам артефакты для обмена данными нам пришлось его программно дополнить следующим образом:<br />
...
После "BuildAndTest" запускается "Delete", который удаляет ветку temp и workflow "Delete" из:
...
После получения всех артефактов программа выводит результат их обработки. В нашем случае все проверки пройдены и мы получаем следующий вывод:
