...
namespace tempApp
{
    class Program
    {
        ...
        {
            string sourseDir = @"C:\testDirectory\sourceDir";//директория, содержащая папку с кодом для тестирования и папку .github с .yml файлом для запуска workflow
            string targetDir = @"C:\testDirectory2\targetDir";//директория, создаваемая для инициализации git репозитория
            string gitRepoDir = @"C:\testDirectory2\targetDir";
            string username = "Sampletext";//данные для коммита
            string email = "Sampletext";//данные для коммита
            string USERNAME = "USERNAME";//имя пользователя для аутентификации
            string PASSWORD = "Personal Access Token";
            string url = "https://github.com/IlMkn/GitHubActionsTest-2";//url github репозитория
            ...
            Repository.Init(gitRepoDir);//git init
            Repository repo = new Repository(gitRepoDir);
            ...
            var di = new DirectoryInfo(@"C:\testDirectory2");

            Delete(di);
            Directory.Delete(@"C:\testDirectory2", true);//удаление временной директории
            ...
            string durl;
            string query;
            query = "https://api.github.com/repos/IlMkn/GitHubActionsTest-2/actions/artifacts";
            Console.WriteLine();
            try
            {
